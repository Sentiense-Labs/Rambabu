...
## RC Car Project Integration

When building Agno agents for this RC car project:

- **Tools must wrap `lib/` classes** (MotorController, Ultrasonic, PanTilt, Camera, Speaker) — each already returns dicts/primitives, matching Agno's tool conventions
- **Respect the two-layer safety system** — never bypass SonarGuard or `POST /motor/front` 409 guard. Agno tool functions must check distance before commanding forward motion
- **Use `Gemini` model class** since the project already uses `gemini-2.5-flash`
- **Store sessions in SQLite** via `agno.db.sqlite.SqliteDb` — matches the project's no-database constraint (file-based only)
- **Import direction still applies** — Agno agent code in `brain/` may import from `lib/` and `config/`, never upward to `server/`

Key differences from current `brain/` implementation:

| Current (brain/) | Agno equivalent |
|---|---|
| Manual Gemini API via `google-genai` | `agno.models.google.Gemini` |
| Hand-built tool schemas in `tools.py` | Auto-generated from function signatures + docstrings |
| Manual function-calling loop in `brain.py` | `Agent.run()` handles the loop |
| `HardwareContext` dataclass | `RunContext.dependencies` or direct injection |
| `brain/memory/observations.md` | `Agent` with `db=` and `add_history_to_context=True` |

Example: wrapping an existing lib/ class as an Agno tool:

```python
from agno.tools import tool
from lib.ultrasonic import Ultrasonic

def get_distance(sensor_id: str) -> dict:
    """Get distance reading from ultrasonic sensor.

    Args:
        sensor_id (str): Which sensor ('front' or 'rear').
    """
    # ultrasonic instance injected via RunContext or globals
    reading = ultrasonic.get_distance()
    return {"distance_cm": reading, "sensor": sensor_id}
```

## Updating

To refresh this skill with updated documentation:
1. Re-run the scraper with the same configuration: `npx @smithery/cli@latest skill add delorenj/agno -a opencode`
2. The skill will be rebuilt with the latest information from docs.agno.com
3. After refreshing, re-add the RC Car Project Integration section above
