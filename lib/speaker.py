...
class Speaker:
    ...
    def say_distance(self, cm: float) -> dict:
        ...
        if distance_cm < config.SAY_DISTANCE_THRESHOLD:
            ...
            return {"status": "ok", "action": "say_distance",
    ...
    def play_wav_async(self, file_path: str) -> dict:
        ...
        thread = threading.Thread(
            target=self.play_wav, args=(file_path,), daemon=True
        )
        thread.start()
        return {"status": "ok", "action": "play_wav_async",
    ...
    def play_mp3_async(self, file_path: str) -> dict:
        ...
        thread = threading.Thread(
            target=self.play_mp3, args=(file_path,), daemon=True
        )
        thread.start()
    ...
    def speak_sync(self, text: str) -> dict:
        ...
        try:
            ...
            return {"status": "error", "error_code": "SPEECH_ERROR",
                    "message": str(exc)}
